...
class Session extends Repository
{
    ...
    {
        $modelClass = config('tracker.session_model');

        list($model, $cacheKey) = $this->cache->findCached($uuid, 'uuid', $modelClass);

        if (!$model) {

            $model = $this->newQuery(new $modelClass())->where('uuid', $uuid)->with($this->relations)->first();
